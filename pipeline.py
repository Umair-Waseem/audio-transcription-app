...
from .config import AUDIO_FILE_TYPES, LANGUAGE_DISPLAY_MAP, SAMPLE_RATE, THREAD_JOIN_TIMEOUT, format_timestamp
...
class ProcessingPipeline:
    ...
    def load_and_prepare(self):
        file_path = filedialog.askopenfilename(filetypes=AUDIO_FILE_TYPES)
        if not file_path:
            return
        try:
            self.stop_and_reset()
        ...
        self.gui.set_button_states(tk.NORMAL, tk.DISABLED)
        self.gui.set_restart_state(tk.NORMAL)
    ...
    def start_processing(self, device: str):
        ...
        self.gui.set_button_states(tk.DISABLED, tk.NORMAL)
    ...
    def _finish_run(self):
        ...
        if self.audio_handler.play_active:
            self._post("button_state", tk.DISABLED, tk.NORMAL)
        else:
            self.run_started = False
            self._post("button_state", tk.NORMAL, tk.DISABLED)
    ...
    def resume_processing(self):
        ...
        try:
            ...
            self.gui.set_button_states(
                tk.NORMAL if self.audio_handler.has_audio() else tk.DISABLED, tk.DISABLED
            )
    ...
    def _on_playback_finished(self):
        ...
        self._post("button_state", play_state, tk.DISABLED)
    ...
    def stop_and_reset(self, keep_audio: bool = False):
        ...
        if not hasattr(self.gui, "play_btn"):
            return
        self.gui.set_button_states(tk.DISABLED, tk.DISABLED)
        self.gui.highlight_button(None)
        if not keep_audio:
            self.gui.set_restart_state(tk.DISABLED)
